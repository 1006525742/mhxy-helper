#!/usr/bin/env python3
"""宝图（分图助手）预测模块 - 与抓鬼预测器同级、独立

功能：
1. 根据坐标在宝图地图上标注位置
2. 预测藏宝可能出现的象限区域（逻辑与抓鬼一致）

地图来源：data/maps_baotu（共 19 张，比抓鬼多 8 张：
北俱芦洲/长寿郊外/大唐国境/花果山/麒麟山/狮驼岭/墨家村/东海湾）
"""
import os
import logging
...
MAPS_DIR = BASE_DIR / "data" / "maps_baotu"

# 地图信息配置
# 11 张与抓鬼共享 + 8 张宝图独有 = 19 张
MAP_CONFIGS = {
    # ===== 与抓鬼共享的 11 张 =====
    '傲来国': {
        'names': ['傲来国', '傲莱国', '傲来', '来国', '傲', '来'],
        'width': 224,
        'height': 150,
        'file': 'alg.png',
        'area_color': (255, 100, 100, 80)
    },
    '宝象国': {
        'names': ['宝象国', '玉象国', '宝象', '象国', '宝', '象'],
        'width': 160,
        'height': 120,
        'file': 'bxg.png',
    ...
    '大唐境外': {
        'names': ['大唐境外', '天唐境外', '大唐', '唐境外', '境外', '大', '唐', '境'],
        'width': 640,
        'height': 120,
        'file': 'dtjw.png',
        'area_color': (255, 0, 0, 128)
...
class BaotuPredictor:
    ...
    def _load_maps(self):
        ...
        logger.info(f"加载 {len(self.maps)} 个宝图地图")
    ...
    def predict_position_areas(self, x: int, y: int, map_info: MapInfo) -> Tuple[List[int], List[int], str, bool]:
        ...
        if x > x_threshold_high:
            x_corner = [1, 4]  # 坐标偏右 → 在右侧角落
        elif x < x_threshold_low:
            x_corner = [2, 3]  # 坐标偏左 → 在左侧角落

        if y > y_threshold_high:
            y_corner = [1, 2]  # 坐标偏上 → 在上侧角落
        elif y < y_threshold_low:
            y_corner = [3, 4]  # 坐标偏下 → 在下侧角落
    ...
    def predict(self, map_name: str, x: int, y: int) -> BaotuPrediction:
        """预测藏宝位置
        ...
        Returns:
            BaotuPrediction 预测结果
        """
        prediction = BaotuPrediction(map_name=map_name, x=x, y=y)
...
# 全局预测器实例
_predictor: Optional[BaotuPredictor] = None

def get_predictor() -> BaotuPredictor:
    """获取预测器单例"""
    global _predictor
    if _predictor is None:
        _predictor = BaotuPredictor()
    return _predictor
...
if __name__ == "__main__":
    ...
    print("支持的宝图地图:", predictor.get_map_list())

    # 测试：北俱芦洲 (100, 80)
    result = predictor.predict("北俱芦洲", 100, 80)
    print(f"\n预测结果: {result.map_name} ({result.x}, {result.y})")
    print(f"可能象限: {result.position_areas}")
    print(f"置信度: {result.confidence}")

    # 测试：大唐国境 (175, 167) - 中间区域
    result2 = predictor.predict("大唐国境", 175, 167)
