#!/bin/bash
# 重启 mhxy 外网链路所需的本机服务：frpc 隧道 + 挖图后端 + 物价手册
# 用法：在 Mac 终端执行  bash ~/mhxy-helper/restart_mhxy.sh
set -e

echo "==> 重启 frpc 隧道（含 watu 8003->8084, wuji 8767->8085）"
...
echo "==> 拉起物价手册后端 (wuji, 8767)"
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.mhxy.wuji.plist 2>/dev/null || true
sleep 1
launchctl kickstart -k gui/$(id -u)/com.mhxy.wuji 2>/dev/null || true
